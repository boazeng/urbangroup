/**
 * close-invoice.js
 * Finalizes a draft invoice in Priority using the Web SDK.
 *
 * Usage:  node close-invoice.js <IVNUM>
...
async function finalizeInvoice(ivnum) {
  ...
  // Build login configuration — use the WCF URL (ends with .svc so SDK won't modify it)
  const loginCfg = {
    url: cfg.wcfUrl,
  ...
  process.stderr.write(`[close-invoice] Logging in to ${cfg.company}...\n`);
  try {
  ...
  // Open AINVOICES form with autoRetrieveFirstRows=1 to load data
  ...
  process.stderr.write(`[close-invoice] Closing ${ivnum}...\n`);
  ...
  await form.endCurrentForm();
  process.stderr.write(`[close-invoice] ${ivnum} closed successfully\n`);
...
if (!ivnum) {
  output({ ok: false, error: 'Usage: node close-invoice.js <IVNUM>' });
  process.exit(1);
}

finalizeInvoice(ivnum)
  .then((result) => {
    output(result);
    process.exit(0);
  })
